...
BigInteger::BigInteger(const Blk *b, Index blen, Sign s) : mag(b, blen)
{
	...
	case zero:
		//swapnibble
		eosio_assert(mag.isZero(), "BigInteger::BigInteger(const Blk *, Index, Sign): Cannot use a sign of zero with a nonzero magnitude");
		// if (!mag.isZero())
...
BigInteger::BigInteger(const BigUnsigned &x, Sign s) : mag(x)
{
	...
	case zero:
		//swapnibble
		eosio_assert(mag.isZero(), "BigInteger::BigInteger(const BigUnsigned &, Sign): Cannot use a sign of zero with a nonzero magnitude");
...
{
	//swapnibble
	eosio_assert(sign != negative, "BigInteger::to<Primitive>: Cannot convert a negative integer to an unsigned type");
...
{
	eosio::print(bigIntegerToString(*this).c_str());
}
